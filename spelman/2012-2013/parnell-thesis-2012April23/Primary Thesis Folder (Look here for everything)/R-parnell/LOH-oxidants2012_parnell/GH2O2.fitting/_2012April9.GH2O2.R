# 2012 April 9, loop over files
# 2012 April 9, log transformation of H2O2

rm(list=ls())
require(nlme) #for gnls
setwd("~/projects/LOH-oxidants2012.osX/GH2O2.fitting")
...
##########
dataFLList = list.files( path="../data/"); dataFLList; 
#dataFLList = dataFLList[1:2] #for debug

i=10
#for( i in 1:length(dataFLList) ) {
 #infile = "_h2o2loh.csv";
infile = paste( '../data/', dataFLList[i], sep=''); infile; 
...
mystart = c(9E-1,23, 2.5); mysd = c(1,2,2); myRuns= 1E2
...
fm <- gnls( s ~ exp( (I/G)* ( 1 - exp(G * t))), start = list( I = myBestIGM[1], G = myBestIGM[2] ) );
#fm3 <- gnls( s ~ exp( (I/G)* ( 1 - exp(G * t)) - M*t), start = list( I=1.3, G=20.6, M=1 ) );
#fm <- gnls( s ~ exp( (I/G)* ( 1 - exp(G * t)) ),start = list( I = 1.3, G = 20.6));

tFit =  seq(min(t), max(t), by=0.005)
#sFit = GM.s( t(myBestIGM), tFit )
#sFit = GM.s( t(fm$coef), tFit) 
sFit = G.s( t(fm$coef), tFit) 

#sFit
plot( s ~ t, type='p', pch=19, main=paste( tb$Strain[1], tb$Date[1],'I=', myBestIGM[1],'G=',myBestIGM[2],'M=',myBestIGM[3] ) ) 
#points( sFit ~ tFit, col="red"); 
lines( sFit ~ tFit, col='red')
#lines( s ~ t, col="blue")
myBestIGM
fm$coef
