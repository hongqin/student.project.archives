 rm( list = ls() );
 tb = read.table("summary.by.strain.csv", header=T, sep="\t");
 tb.old = tb;
 labels = names( tb.old );
 #tb = tb.old[c(1:11), c(1:5,8, 10, 12, 14, 16, 18, 20, 22)]
 tb = tb.old[c(1:11), c("strain","ARLS","R0","G","CLS","Tc", "Tg","Tmmax","Tbmax", "Td", "Tdmax", "b.max", "b.min", "strains", "L0.all", "L0.small" , "Pbt0","Pb0.5t0", "Pbt0.b") ];

#########110706Tue regressions
summary( lm( tb$L0.small ~ tb$ARLS + tb$Tc + tb$CLS + tb$R0 + tb$G ) ) #p 0.45

summary( lm( tb$L0.all ~ tb$ARLS + tb$Tc + tb$CLS + tb$R0 + tb$G ) ) #p 0.47
# only CLS shows connection

summary( lm( tb$L0.small ~ tb$ARLS + tb$CLS) ) #p 0.55

summary( lm( tb$L0.all ~ tb$ARLS) ) #p 0.49

summary( lm( tb$L0.all ~ tb$ARLS + tb$CLS) ) #p 0.05

summary( lm( tb$L0.all ~ tb$CLS) ) #p 0.013  <--

summary( lm( tb$L0.all ~ tb$Tc) ) #p 0.35

summary( lm( tb$L0.all ~ tb$Tg) ) #p 59           

summary( lm( tb$L0.all ~ tb$Tbmax )) #p 0.69
