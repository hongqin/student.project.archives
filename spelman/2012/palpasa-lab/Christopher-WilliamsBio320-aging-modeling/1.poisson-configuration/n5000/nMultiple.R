...
for( i in 1: Npop){  # i-th individual in the population
  ...
  for( j in 1:m) {#Block loop
    ...
    #maximum of elelementAges -> BlockAges
...
write.table(t(out), file=paste('n', nIn, '.csv', sep=''), append=TRUE, sep=',', row.names=F, col.names=F)  
#want to generate multiple values for the slope (number of working components is randomized)
plot( log(sub$m[1:30]) ~ sub$t[1:30], main=nIn)
abline( m , col='red')
